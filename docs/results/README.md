...
| `firstboot.txt` | the real v3 desktop image under OVMF with NVRAM that persists: it asks "does this work?", yes makes AurOS the default, no sends the next start to Windows and nothing re-arms, and -- case C -- with a second entry called AurOS in the menu it takes its own, by the partition it names. 32 checks. | `sudo sh tools/firstboottest.sh desktop` |
| `putback.txt` | "Put Windows back", from the button in the real v3 image with Secure Boot on: the word as the desktop user, `answer.sh` in its real sandbox, a restart, AurOS's own menu taking the restore entry, grub starting the staging kernel with `aurstage.restore`, and the next start AurOS again. 16 checks. | `sudo sh tools/putbacktest.sh` |
