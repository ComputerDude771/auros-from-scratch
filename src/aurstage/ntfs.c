...
/* The declared size of a non-resident attribute's data, or 0. */
static uint64_t nonres_size(const unsigned char *a)
{ return le64(a + 0x30); }
                       ...
                       uint32_t csize, uint64_t voff, void *buf, size_t n)
{
    ...
    while (rl_next(&rl, &count, &lcn)) {
        ...
        uint64_t run_hi = run_lo + count * csize;
        vcn += count;
 ...
 * this node", -2 for "this node does not make sense". */
static int64_t node_walk(const unsigned char *nh, size_t avail, const char *name)
{
                           ...
                           uint32_t bps, uint32_t csize, const char *name)
{
    ...
    int64_t hit = node_walk(nh, irlen - 0x10, name);
    ...
    for (uint64_t off = 0; off < total; off += blk) {
        ...
        int64_t h = node_walk(ib + 0x18, blk - 0x18, name);
        if (h >= 0) { r = h; break; }
        if (h == -2) { r = -2; break; }
    }
    free(ib);
...
{
    int64_t n = root_lookup(fd, mft_off, mft_rec, bps, csize, "hiberfil.sys");
    ...
    if (nonres_size(da) < 4096) { r = NTFS_NO; goto out; }
                              ...
                              uint32_t bps, uint32_t csize)
{
    ...
    if (nonres_size(da) < 8192) goto out;
...
void ntfs_read_state(const char *dev, ntfs_state *st)
{
    ...
    uint32_t mft_rec = (cpr >= 0) ? (uint32_t)cpr * st->bytes_per_cluster
                                  : (uint32_t)1u << (uint32_t)(-cpr);
    ...
    close(fd);
    st->verdict = NTFS_OK;
    if (st->hibernated == NTFS_UNSURE || st->log_dirty == NTFS_UNSURE)
