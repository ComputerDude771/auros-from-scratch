...
| `notifytest.c` | The notification server, driven over a **real session bus started by the test** with a **real client** (`gdbus`) — not by calling its handlers. Proves a program's message reaches her screen, that pressing a card removes it, that six at once do not become six cards, and that nothing a sending program controls can take over the screen: a 2000-character summary, markup, control characters, a malformed call. Also measures every card at three resolutions × four text sizes against the 44px floor. |
